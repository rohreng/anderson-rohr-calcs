...
log(`MASONRY AUDIT source=84af963 chrome=${chromeVersion}`);
log(`Fixtures expected=${fixtures.length} calculators=${new Set(fixtures.map(f=>f.calc)).size}`);
let harnessErrors=0, mismatches=0; const manifest=[];
for(const f of fixtures){
  ...
  const failed=result?result.assertions.filter(a=>!a.pass):[]; mismatches+=failed.length;
  ...
  const state=broken?'HARNESS_ERROR':failed.length?'FINDING_MISMATCH':'EXECUTED';
  log(`${state} ${key}`);
  for(const a of (result?.assertions||[])) log(`  ${a.pass?'PASS':'FAIL'} ${a.label}: actual=${a.actual} expected=${a.expected}`);
  manifest.push(record);
}
const gate=harnessErrors===0&&manifest.length===fixtures.length&&new Set(manifest.map(x=>x.key.split('/')[0])).size===10;
const summary={schema_version:1,source_commit:'84af963',chrome_version:chromeVersion,expected_fixtures:fixtures.length,executed:manifest.length,calculators:10,benchmark_mismatches:mismatches,harness_errors:harnessErrors,aggregate_gate:gate?'PASS':'FAIL',gate_meaning:'Completeness and harness health; benchmark mismatches are audit findings in this pre-fix phase.',records:manifest.map(x=>({key:x.key,benchmark_status:x.benchmark_status,harness_error:x.harness_error,screenshot:x.screenshot}))};
fs.writeFileSync(path.join(ev,'manifest.json'),JSON.stringify(summary,null,2));
log(`AGGREGATE ${gate?'PASS':'FAIL'} executed=${manifest.length}/${fixtures.length} calculators=${new Set(manifest.map(x=>x.key.split('/')[0])).size}/10 findings=${mismatches} harness_errors=${harnessErrors}`);
