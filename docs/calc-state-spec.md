...
| `rectangular_diaphragm_calculator.html` (engine `public/Calcs/engines/rect-diaphragm.js`) | — (Tier A, no adapter) | — | Ordinary Tier-A fields; the carriers that matter downstream are `#swJSON` (the shearwall rows `{X:[{label,len,loc}], Y:[…]}`, authoritative since Phase 0), `#mwfrsJSON` (the `are.lateral.v1` story table the MWFRS send leaves behind, blank when the level was entered by hand) and `#loadLevel` (`strength` \| `asd`, how `#Vx #Vy #Vx_s #Vy_s` are to be read). | A page-local `AREv2.loadFromState` wrapper strips the legacy per-row keys `#sw{X\|Y}_{label\|len\|loc}_{n}` (a counter never re-indexed on delete, so they could not match key-for-key) and defaults `#loadLevel` (`strength`) and `#mwfrsJSON` (`''`) for files saved before they existed. `are-utils-v2.js` is not modified. |
