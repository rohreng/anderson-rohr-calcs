...
| `stacked-shearwall` (`public/Calcs/engines/stacked-shearwall.js`, `SW.ENGINE.version = 2`) | 2 | `#floor-con` | `version`, `floors` (each: `id, name, h_ft, P_wind_lb, P_seis_lb, walls[] {id, label, L_ft, P_wind_lb, P_seis_lb (null = inherit level force), dir, loc_ft (Phase 4 import keys, not written by the page), segments_ft[], openings[], unsheathed_ft2, sheathing{face1, face2, blocked, insideFaceHoldown}, endPost{n,size}, holdown, sill{conn, spacing_in}, sillSpecies, dead{w_plf, P_end_lb, source}, transfer}`), `wCnt`, `lateral` (`{schema: 'are.lateral.v1', project, dir: 'X'|'Y', files[] (basenames, ≤ 120 chars, no `<>`), importedAt (ISO)}` — import provenance written by the page's "Import diaphragm levels" panel via `LH.toShearwallState`; `null` when hand-built; rendered as a muted line at the top of `#floor-con`); page-level Tier-A `#sfrs #sdc #species` | v1 records are **refused** (engine `SW.V1_REFUSAL`, same string in the JSON import and the adapter); no legacy records existed. Files saved before the per-wall line force carry no `P_wind_lb`/`P_seis_lb`/`dir`/`loc_ft` on walls and no `lateral`; absent = inherit the level force, and `allowedKeys` is a whitelist, not a required list. |
| `wood-connection-schedule` (`public/Calcs/engines/wood-connections.js`, `WC.ENGINE.version = 1`) | 1 | `#schedule` | `version`, `bolts`, `nails`, `screws` (each row: `id, desc, V, T, loadCase, cmException, main{species, t, w, theta, endGrain, towardEnd, endDist, edgeDist, loadedEdgeDist}, side{mat, species, t, w, theta, towardEnd, endDist, edgeDist, loadedEdgeDist, gauge}, n, rows, s, g, shrinkDetail, Fyb_override, notes` plus per type `D, shear, mainSteel` / `nailType, penny, toeNail, diaphragm` / `screwType, no, D, L`; contract in `docs/superpowers/specs/2026-09-19-wood-connection-schedule-spec.md` §3), `rowCnt` (monotonic row id, never reused); `header` is an allowed key but is **not** written by `getModel()` — the header fields are page-level Tier-A `#species #E_override #custom_* #steelGrade #mcFab #mcService #temp` outside `#schedule`, and `runAndSettle()` / `setModel()` re-read them from the DOM | `setModel` refuses `version !== 1`; no legacy records existed (first shipped 2026-09-19). Row `desc` / `notes` ≤ 120 chars, no `<>` (schema `stringPattern`). |
| `rectangular_diaphragm_calculator.html` (engine `public/Calcs/engines/rect-diaphragm.js`) | — (Tier A, no adapter) | — | Ordinary Tier-A fields; the carriers that matter downstream are `#swJSON` (the shearwall rows `{X:[{label,len,loc}], Y:[…]}`, authoritative since Phase 0), `#mwfrsJSON` (the `are.lateral.v1` story table the MWFRS send leaves behind, blank when the level was entered by hand) and `#loadLevel` (`strength` \| `asd`, how `#Vx #Vy #Vx_s #Vy_s` are to be read). Parapet steps (2026-09-23, engine v2): `#ppQp #ppHmax #ppHtyp` (source-owned, from `record.parapet`), `#ppRoof` (`flat` \| `sloped` \| `''`), `#ppUnlock #ppCommonBase` (checkboxes, captured as booleans), `#stepsAtLevel` (`on` \| `off`) and `#stepJSON` (authoritative `[{label, face, start_ft, width_ft, h_ft}]`; row inputs are `data-are-ignore`). `LH.parapetFromFields` reads exactly these. | A page-local `AREv2.loadFromState` wrapper strips the legacy per-row keys `#sw{X\|Y}_{label\|len\|loc}_{n}` (a counter never re-indexed on delete, so they could not match key-for-key) and defaults `#loadLevel` (`strength`), `#mwfrsJSON` (`''`) and, for files saved before the parapet steps existed, `#stepJSON` (`[]`), `#stepsAtLevel` (`on`), `#ppUnlock` / `#ppCommonBase` (`false`) and the source-owned parapet values (kept from the page only when the page's story table is kept). `are-utils-v2.js` is not modified. |

### `are.lateral.v1` — the lateral handoff record

`public/Calcs/engines/lateral-handoff.js` (`LH`) is the **source of truth** for the record that carries
MWFRS → Rectangular Diaphragm → Stacked Shearwall; this paragraph is a pointer, not a second definition.
`schema: 'are.lateral.v1'`; `loadLevel: 'strength'` always (an ASD diaphragm file is converted on read, /0.6
wind, /0.7 seismic); units live in the field names (`*_strength_lb`, `sh_ft`, `loc_ft`); `levels[]` are
**top → bottom**; a wall's identity across levels is `id = dir@loc` (`X@15`, `Y@45.3`, loc rounded to 0.1 ft),
never its label. The record is staged in `localStorage.are_lateral_v1` as `{record, ts, file}` for the two
quick sends (10-minute TTL, consumed on read by the page named in `file`) and is otherwise rebuilt from saved
diaphragm files by the stacked shearwall's import panel (`AREv2.parseSnapshot` → `LH.levelFromDiaphragmState`
→ `LH.assemble` → `LH.toShearwallState`). Tests: `npm run test:lat`, `test:dia`, `test:mwfrs`, `test:sw`.
