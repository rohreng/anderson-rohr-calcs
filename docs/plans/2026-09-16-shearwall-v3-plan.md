...
- **Copy-down (E) as implemented** — `copyWallsDown(fi)`, page only, header button `⇩ Copy walls to levels below` on every level but the base (the engine's segmented count-mismatch error names it):
  ...
  - Base (`applyBaseRules`, targets and line mates alike): existing `sill` kept (`ab58 @ 20` for a just-created wall); existing `sillSpecies` kept (a just-created base wall takes the wall above's); `holdown` `strap` → `hdue` (validate: straps are floor-to-floor only); `uplift.penetration_in` → `null` (= `SW.penetrationDefault(sc, true)`, no subfloor) — a typed penetration never crosses a floor; `uplift.washer_in` kept (it belongs to the base's anchor bolts); at an anchor-bolt sill (`SW.findSill(conn).base`) a Manual `uplift.source` reverts to `'sill'` (a plf typed for a nailed / screwed plate is connector-specific) with `capacity_plf` / `label` left in the object so switching back to Manual restores them.
  ...
  - Engine (the one Phase E engine change): `validate` warns once per wall id whose `lineKey` differs between levels — "Wall <id> is on line "<a>" at <level> but line "<b>" at <level> — the line key should match on every level (the copy-down button copies it)." Fixture SW67 (roof A + B on "L", base A implicit → warning; same keys → none); SW1–SW66 `runFixtures()` lines identical. Print CSS hides `button.btn-undo`.
  - Harness block "copy walls to levels below (Phase E)" in `tools/test-stacked-shearwall.mjs` (13 checks incl. print): roof edited to segmented [100, 68, 4] / two openings / 10d @ 4 / strap / sill @ 8 / p 2.0 / 3 posts / SP, split `w1` + `w1#2`, 2ND-floor `w1` deleted, base-only mate `w2` on line `w1`; copy → every lower `w1` matches, base `ab58/20` / HDUE / p null / washer kept / dead and P_W kept, 2ND `w1` recreated blank, `w1#2` on every level, mate takes the construction, no error, Undo exact and one-step, no dialogs.
